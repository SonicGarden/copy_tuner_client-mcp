...
- **search_key**: Rails i18nキーの検索（`t()`や`I18n.t()`で使用されるキーの検索に最適化）。翻訳済みのキーに加え、登録済みだが訳が空のキーも返す（空文字列 `""` で表現）。空文字列で返ってきたキーはすでに登録済みなので、`create_i18n_key` ではなく `update_i18n_key` で翻訳を設定する
